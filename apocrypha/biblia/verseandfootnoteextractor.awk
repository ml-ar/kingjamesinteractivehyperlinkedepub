...
#2: Could not parse out the chapter and verse from the line that describes the book, chapter, and verse
#3: Could not find the text header in the xhtml
...
if(!match(book,/\s*([[:digit:]]+)\s*:\s*([[:digit:]]+)/,matchArray))
{
print "Error parsing out chapter and verse."; exit 2
}
...
choppedVerse = substr($0, pointer)
choppedVerse = gensub(/(\s*\n\s*)|(^\s*)|(\s*$)/,"","g",choppedVerse); #getting rid of all newspaces and indentations because they're unnecessary
